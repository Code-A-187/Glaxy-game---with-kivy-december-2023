...
class MainWidget(RelativeLayout):
    ...
    def check_ship_cpllision(self):
        for i in range(0, len(self.tiles_coordinates)):
            ti_x, ti_y = self.tiles_coordinates[i]
            if ti_y > self.current_y_loop +1:
                return False
            if self.check_ship_collision_with_title(ti_x, ti_y):
                return True
        return False

    def check_ship_collision_with_title(self, ti_x, ti_y):
    ...
    def update(self, dt):
        ...
        if not self.check_ship_cpllision() and not self.state_game_over:
